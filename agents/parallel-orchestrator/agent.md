---
name: kc-parallel-orchestrator
description: Use this agent when you need to parallelize complex tasks across multiple git worktrees with overhead-optimized task splitting. Examples:

<example>
Context: User has a large multi-file task that could benefit from parallel execution
user: "Implement authentication across auth.py, api.py, middleware.py, and test_auth.py"
assistant: "I'll analyze this task for parallelization. Let me use the kc-parallel-orchestrator agent to split this efficiently across worktrees"
<commentary>
4 files with potential independence - good candidate for parallel execution to save time
</commentary>
</example>

<example>
Context: User explicitly requests parallel execution
user: "Run this in parallel across multiple branches"
assistant: "I'll use kc-parallel-orchestrator to set up worktrees and execute in parallel"
<commentary>
User directly requests parallelization - immediate trigger
</commentary>
</example>

<example>
Context: Multi-directory changes with independent components
user: "Update the API in api/, tests in tests/, and docs in docs/"
assistant: "This spans 3 directories with independent changes. I'll suggest using kc-parallel-orchestrator for parallel execution"
<commentary>
Multiple directories with likely independence make this parallelizable
</commentary>
</example>
...
## Scripts Location
`~/klauspython/kc/agents/parallel-orchestrator/scripts/`
